...
@section('content')
    ...
    <div class="container-fluid">
        <div class="col-sm-7 col-sm-offset-1">
            <h1 class=""><strong>Sport</strong></h1>
            ...
            <div class="row">
                <div class="col-sm-12">
                    @foreach($une as $detail)
                        <a href="{{route('rubric.show', [$detail->rubric, $formatData($detail->start_at), $detail->id, $detail->slug])}}">
                            <h4>{{$detail->title}}</h4>
                            <img src="https://placehold.it/350x220?text=IMAGE" class="img-responsive img-thumbnail" style="width:100%" alt="Image">
                            <p>{{$detail->content}}</p>
                        </a>
                    @endforeach
                </div>
                        ...
                        <a href="{{route('rubric.show', [$detail->rubric, $formatData($detail->start_at), $detail->id, $detail->slug])}}">
                            <div class="media">
                                ...
                                <div class="media-right">
                                    <img class="media-object" src="images/gvbuyb.jpg" alt="..." style="">
                                </div>
                            </div>
                        </a>
                    @endforeach
            ...
            <div class="col-sm-12" style="background-color: #666; padding: 10px; border-radius: 10px; margin-bottom: 20px;">
                <form method="post" action="" id="newslettres">
                    <div class="form-group form-group-sm col-sm-12">
                        <label for="newslettres" class="">Newsletter Économie</label>
                        <input type="email" name="" class="form-control" placeholder="Insèrez votre email" id="newslettres">
                    </div><br>
                    <button class="btn btn-defaultcol-md-4 col-xs-10" style="margin-left: 15px;" type="submit" name="" >Envoyer</button>
                </form>
            </div>
